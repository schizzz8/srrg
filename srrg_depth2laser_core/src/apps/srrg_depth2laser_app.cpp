...
int main(int argc, char ** argv) {
    ...
    while ((msg = reader.readMessage())) {
        ...
        if (pinhole_image_msg){
            if(!got_info){
                extractor.setK(pinhole_image_msg->cameraMatrix());
                cerr << "Camera Matrix:" << endl << extractor.K() << endl;
                extractor.setCameraTransform(pinhole_image_msg->offset());
                Eigen::Vector3f camera_translation = extractor.cameraTransform().translation();
                Eigen::Quaternion<float> camera_rotation(extractor.cameraTransform().rotation());
                ...
                extractor.setLaserTransform();
                Eigen::Vector3f laser_translation = extractor.laserTransform().translation();
                Eigen::Quaternion<float> laser_rotation(extractor.laserTransform().rotation());
